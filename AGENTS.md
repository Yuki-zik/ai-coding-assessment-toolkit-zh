...
轻量项目记录要求：
在项目根目录维护 `agent/` 目录（存放运行态记录）并维护根目录 `AGENTS.md`（统一代理规范）。若不存在，则创建以下文件：
- `agent/project.md`：项目目标、archetype、成功标准、非目标、关键约束
- `agent/tasks.md`：当前任务拆分、状态、下一步
- `agent/timeline.md`：关键决策、重要修改、验证结果、问题与修复
- `AGENTS.md`：项目根目录唯一代理工作说明（本项目中的工作方式、约束、答题策略摘要）

维护规则：
1. 会话开始时先做“最新窗口读取”，不要全量读取四个文件：
   - `agent/tasks.md`：仅读进行中/下一步/最高优先级条目
   - `agent/timeline.md`：仅读顶部最近 3-5 条
   - `agent/project.md` 与根目录 `AGENTS.md`：仅在约束不清、规则冲突或架构变更时按需读取相关段落
...
补充：只有当最新窗口不足以支持安全决策时，才回溯更早历史记录。

并行与子 agent 规则（执行层）：
- 优先并行独立子任务：仓库检索、文档核对、测试排查、互不重叠的代码片段实现。
- 并行时必须明确文件所有权，禁止多个子 agent 同时改同一核心业务文件。
- 收口由主 agent 负责，避免并行后缺少统一决策与集成验证。
- 子 agent 推理强度按复杂度选择：
  - `low`：检索、摘录、快速比对
  - `medium`：常规实现与测试补齐
  - `high`：跨模块改动、疑难故障定位
  - `xhigh`：架构取舍、安全边界与高不确定性问题（默认）
- 采用“先低后高”策略：先低成本试探，证据不足再升级。
